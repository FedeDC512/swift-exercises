...
// foo is defined inside the loop, so it’s not accessible outside it, causing a compiler error.
// x is defined outside, allowing it to be modified inside the loop and accessed afterward.
/*:
...
func greeting(greeting: String?, name: String) {
    guard let unwrappedGreeting = greeting else {
        print("Hello, \(name).")
        return
    }
    print("\(unwrappedGreeting), \(name).")
}
greeting(greeting: "Hi there", name: "Fede")
greeting(greeting: nil, name: "Fede")
/*:
...
let myCar = Car(make: "Alfa Romeo", model: "Giulietta", year: 2010)
