...
func checkMeals (userMeal:String) -> Meal? {
    if let value = meals[userMeal] {
        return meals[userMeal]
    } else {
        return nil
    }
}
print(checkMeals(userMeal: "Breakfast"))
print(checkMeals(userMeal: "Lunch"))
/*:
...
func checkMealLog() -> [String: Any]? {
    if let myDictionary = UserDefaults.standard.dictionary(forKey: "mealLog") {
        return myDictionary
    } else {
        let newDictionary:[String: Any]? = nil
